...
- A clean build at `vYYYY.M.D` uses `YYYY.M.D`.
...
- Before the first release, `package.json` supplies the bootstrap date and the repository's total commit count supplies the development number.
...
Release a clean commit by tagging it with its publication date. Do not manually update a development counter.
